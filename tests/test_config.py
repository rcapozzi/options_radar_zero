...
class TestAppConfig:
    def test_defaults(self):
        ...
        assert config.DATA_DIR == '../tda-tbd/wip'
...
class TestGetParquetFiles:
    ...
    def test_with_parquet_files(self, tmp_path):
        ...
        (tmp_path / "SPX.X.2024-01-01.parquet").touch()
        (tmp_path / "SPY.2024-01-01.parquet").touch()
        (tmp_path / "chain.2024-01-01.parquet").touch()  # should be skipped

        with patch('options_radar_zero.config.config') as mock_config:
            mock_config.DATA_DIR = str(tmp_path)
            result = get_parquet_files()
            assert 'SPX.X.2024-01-01' in result
            assert 'SPY.2024-01-01' in result
            assert 'chain.2024-01-01' not in result

    def test_max_files_limit(self, tmp_path):
        """Test that max_files parameter limits results."""
        for i in range(25):
            (tmp_path / f"SYM.{i}.parquet").touch()
    ...
    def test_sorts_reverse(self, tmp_path):
        ...
        (tmp_path / "AAA.2024-01-01.parquet").touch()
        (tmp_path / "ZZZ.2024-01-01.parquet").touch()
